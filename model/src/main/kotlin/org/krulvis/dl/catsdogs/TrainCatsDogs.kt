...
import org.krulvis.dl.catsdogs.CatsDogs.EPOCHS
import org.krulvis.dl.catsdogs.CatsDogs.IMAGE_SIZE
import org.krulvis.dl.catsdogs.CatsDogs.PATH_TO_MODEL
import org.krulvis.dl.catsdogs.CatsDogs.TEST_BATCH_SIZE
import org.krulvis.dl.catsdogs.CatsDogs.TRAINING_BATCH_SIZE
import org.krulvis.dl.catsdogs.CatsDogs.TRAIN_TEST_SPLIT_RATIO
import org.krulvis.dl.preprocessing.Convert
import java.io.File

object CatsDogs {
    const val PATH_TO_MODEL = "cache/savedmodels/vgg11"
    const val EPOCHS = 3
    const val TRAINING_BATCH_SIZE = 32
    const val TEST_BATCH_SIZE = 256
    const val NUM_LABELS = 2
    const val NUM_CHANNELS = 3L
    const val IMAGE_SIZE = 64L
...
fun main() {
    ...
    CatsDogs.vgg11.use {
        ...
        it.saveModelConfiguration(
            File("$PATH_TO_MODEL/modelConfig.json")
        )
        it.logSummary()
